...
// Mouse implementation based on working reference code
...
/// Initialize mouse input - matches working reference
pub fn initMouse(boot_services: *uefi.tables.BootServices) ?*uefi.protocol.SimplePointer {
    if (boot_services.locateProtocol(uefi.protocol.SimplePointer, null)) |mouse_result| {
        if (mouse_result) |m| {
            // Reset with ExtendedVerification=true (like working code)
...
/// Get mouse event for waiting - validates pointer
...
/// Update mouse state from UEFI input - matches working reference
pub fn updateMouse(state: *MouseState, mouse: ?*uefi.protocol.SimplePointer, screen_w: u32, screen_h: u32) void {
    if (mouse) |m| {
        if (m.getState()) |ms| {
            // Use same divisor as working code (5000)
            const dx = @divTrunc(ms.relative_movement_x, 5000);
            const dy = @divTrunc(ms.relative_movement_y, 5000);
